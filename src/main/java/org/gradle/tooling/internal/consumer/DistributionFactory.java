...
public class DistributionFactory {
  ...
  public Distribution getDefaultDistribution(File projectDir, boolean searchUpwards) {
    // deenu modify: don't excute wrapper
    /**
     * BuildLayout layout = new BuildLayoutFactory().getLayoutFor(projectDir, searchUpwards);
     * WrapperExecutor wrapper = WrapperExecutor.forProjectDirectory(layout.getRootDirectory()); if
     * (wrapper.getDistribution() != null) { return new
     * ZippedDistribution(wrapper.getConfiguration(), clock); }
     */
  ...
  public Distribution getDistribution(URI gradleDistribution) {
     ...
     * configuration.setDistribution(gradleDistribution); return new
     * ZippedDistribution(configuration, clock);
     */
  ...
  public static class ZippedDistribution implements Distribution {
    ...
    public ClassPath getToolingImplementationClasspath(
      ...
      if (installedDistribution == null) {
        final DistributionInstaller installer =
            new DistributionInstaller(
                progressLoggerFactory,
                progressListener,
                clock,
                wrapperConfiguration.getNetworkTimeout());
        File installDir;
        try {
  ...
  private static class ClasspathDistribution implements Distribution {
    ...
    public ClassPath getToolingImplementationClasspath(
      ...
      /**
       * DefaultModuleRegistry registry = new DefaultModuleRegistry(null); return
       * registry.getModule("gradle-tooling-api-provider").getAllRequiredModulesClasspath();
       */
      return ClasspathUtil.getClasspath(getClass().getClassLoader());
    }
  }

  // deenu modify: add DefaultDistribution
