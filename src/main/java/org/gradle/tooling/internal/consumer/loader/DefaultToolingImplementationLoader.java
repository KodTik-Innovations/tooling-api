 ...
 * "provider"). Adapts the rather clunky cross-version interface to the more readable interface of
 * the TAPI client.
 */
public class DefaultToolingImplementationLoader implements ToolingImplementationLoader {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(DefaultToolingImplementationLoader.class);
  ...
  public ConsumerConnection create(
    ...
    ClassLoader serviceClassLoader =
        createImplementationClassLoader(
            distribution,
  ...
  private ClassLoader createImplementationClassLoader(
      ...
      ConnectionParameters connectionParameters,
      BuildCancellationToken cancellationToken) {
    // deenu modify: return classLoader
    /**
     * ClassPath implementationClasspath =
     * distribution.getToolingImplementationClasspath(progressLoggerFactory, progressListener,
     * connectionParameters, cancellationToken); LOGGER.debug("Using tooling provider classpath:
     * {}", implementationClasspath); FilteringClassLoader.Spec filterSpec = new
     * FilteringClassLoader.Spec(); filterSpec.allowPackage("org.gradle.tooling.internal.protocol");
     * filterSpec.allowClass(JavaVersion.class); FilteringClassLoader filteringClassLoader = new
     * FilteringClassLoader(classLoader, filterSpec); return
     * VisitableURLClassLoader.fromClassPath("tooling-implementation-loader", filteringClassLoader,
